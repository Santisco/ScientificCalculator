...
public class Parser {
	...
	public void bracket() throws SqrtException, DividedByZeroException,
			ExpressionException {
		int w, q, e;
		Token t1, t2, t0, tj, t3, a1, a2, a3, a4, a5;
		

		// 取stack顶部元素
		t1 = stack.get(index);
		t0 = stack.get(index - 1);
		tj = stack.get(index - 2);
		stack.remove(index);
		index--;
		// 若为()则直接返回空
		double tz = 0, tz1;
		...
		if (t0.getType() == 14) {
			...
			case 10:
				...
				break;
			case 11:			
					tz = Math.sin(Math.toRadians(tz));
				break;
			case 12:
					tz = Math.cos(Math.toRadians(tz));
					if (tz == 6.123233995736766E-17) {
						tz = 0;
					}
			...
			case 29:
				...
				break;
			case 30:
					tz = Math.sinh(Math.toRadians(tz));
				break;
			case 31:
					tz = Math.cosh(Math.toRadians(tz));
				break;
			case 32:
					tz = Math.tanh(Math.toRadians(tz));
				break;
			// ln()运算
			case 34:
					tz = Math.log(tz);
			...
			stack.remove(index);
			index--;
			Value1 t9 = new Value1(tz);
			push(t9);
			for(int m = 0; m < stack.size(); m++){
				System.out.println(stack.get(m));
			}
			System.out.println("应用的是此方法");
			return ;
		}
	...
	public void guiyue() throws SqrtException, EmptyExpressionException,
		...
		while (true) {
			...
			case 0:
				push(t2);
				System.out.println("0操作");
				break;
			// 若两个字符在table表中对应为1，则执行一元操作
			case 1:
				unary(t2);
				System.out.println("非通过branket已经实现一元操作");
				break;
			// 若两个字符在table表中对应为3，则执行处理括号操作
			case 3:
				System.out.println("3操作");
				bracket();
				break;
			// 若两个字符在table表中对应为4，则执行accept函数
			case 4:
				System.out.println("4操作");
				tt = 2;
				while (tt == 2) {
					tt = accept(t2);
				}
